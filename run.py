    ...
    for step, batch in enumerate(dataloader):
        ...
        if args.architecture == 'cross':
            ...
            with torch.no_grad():
                ...
                # loss = F.cross_entropy(logits, torch.argmax(labels_batch, 1))
                mask = torch.eye(labels_batch.size(0)).to(logits.device)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--output_dir", type=str, default='output_quora')
    parser.add_argument("--train_dir", default='quora', type=str)
    ...
    parser.add_argument("--print_freq", default=20, type=int, help="Log frequency")
    ...
    tokenizer = TokenizerClass.from_pretrained(os.path.join(args.bert_model, 'vocab.txt'))
    ...
    concat_transform = SelectionConcatTransform(tokenizer=tokenizer,
                                                max_len=args.max_response_length + args.max_contexts_length)
    ...
    if not args.eval:
        train_dataset = SelectionDataset(os.path.join(args.train_dir, 'train.txt'),
                                         context_transform, response_transform, concat_transform, sample_cnt=None,
                                         mode=args.architecture)
        val_dataset = SelectionDataset(os.path.join(args.train_dir, 'dev.txt'),
                                       context_transform, response_transform, concat_transform, sample_cnt=1000,
                                       mode=args.architecture)
        train_dataloader = DataLoader(train_dataset, batch_size=args.train_batch_size,
                                      collate_fn=train_dataset.batchify_join_str, shuffle=True, num_workers=0)
        t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs
        print('train date size:%d, dev date size:%d' % (train_dataset.__len__(), val_dataset.__len__()))
    ...
    val_dataloader = DataLoader(val_dataset, batch_size=args.eval_batch_size, collate_fn=val_dataset.batchify_join_str,
    ...
    eval_freq = min(len(train_dataloader) // 2, 60)
    ...
    for epoch in range(epoch_start, int(args.num_train_epochs) + 1):
        ...
        with tqdm(total=len(train_dataloader) // args.gradient_accumulation_steps) as bar:
            for step, batch in enumerate(train_dataloader):
                ...
                if args.architecture == 'cross':
                    text_token_ids_list_batch, text_input_masks_list_batch, text_segment_ids_list_batch, labels_batch = batch
                    loss = model(text_token_ids_list_batch, text_input_masks_list_batch, text_segment_ids_list_batch,
                                 labels_batch)
                else:
                    context_token_ids_list_batch, context_input_masks_list_batch, \
                    response_token_ids_list_batch, response_input_masks_list_batch, labels_batch = batch
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if nb_tr_steps and nb_tr_steps % print_freq == 0:
                        ...
                        print('global_step:%d, loss:%.4f' % (global_step, tr_loss / nb_tr_steps))
