...
class BiEncoder(BertPreTrainedModel):
    ...
    def forward(self, context_input_ids, context_input_masks,
        ...
        # print(torch.max(context_input_ids))
        # print(self.bert.embeddings.word_embeddings.weight.size()[0])
        # print('----')
        context_vec = self.bert(context_input_ids, context_input_masks)
        context_vec = context_vec[0][:, 0, :]  # [bs,dim]
        ...
        responses_vec = responses_vec[0][:, 0, :]  # [bs,dim]
...
class PolyEncoder(BertPreTrainedModel):
    ...
    def dot_attention(self, q, k, v):
        """
       计算：poly_codes_embs:
       input: q:[bs, poly_m, dim], k=v:[bs, max_len, dim]
       attn_weights: [bs,poly_m,max_len]
       output: [bs,poly_m,max_len] * [bs, max_len, dim] = [bs,poly_m,dim]
       -------
       计算ctx_emb：
       input: q:[bs,bs,dim],k=v:[bs,poly_m,dim]
       attn_weights: [bs,bs,poly_m]
       output : [bs, bs, poly_m] * [bs,poly_m,dim]=[bs,bs,dim]
       """
        attn_weights = torch.matmul(q, k.transpose(2, 1))
        attn_weights = F.softmax(attn_weights, -1) # sum(attn_weights[0][0])=1,softmax不改变形状，只在某一维度计算（这里在最后一维），让和为1
    ...
    def forward(self, context_input_ids, context_input_masks,
        ...
        # context encoder
        ctx_out = self.bert(context_input_ids, context_input_masks)[0]  # [bs, max_len, dim]
        poly_code_ids = torch.arange(self.poly_m, dtype=torch.long).to(context_input_ids.device)  # 生成poly_ids, [poly_m]
        ...
        # response encoder
        responses_input_ids = responses_input_ids.view(-1, seq_length)      # [bs,max_len]
        responses_input_masks = responses_input_masks.view(-1, seq_length)  # [bs,max_len]
        cand_emb = self.bert(responses_input_ids, responses_input_masks)
        cand_emb = cand_emb[0][:, 0, :]  # 取cls做为cand_emb，[bs, dim]
        ...
        if labels is not None:
            ...
            dot_product = (ctx_emb * cand_emb).sum(-1)  # [bs, bs],求点积

            mask = torch.eye(batch_size).to(context_input_ids.device)  # 对角线为1的矩阵，[bs, bs]
            loss = F.log_softmax(dot_product, dim=-1) * mask   # 只有对角线才是真正a pair数据的loss
            loss = (-loss.sum(dim=1)).mean()
            return loss
        else:
